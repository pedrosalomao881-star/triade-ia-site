...
export async function POST(request: Request) {
  ...
  console.log(`[creditOS:cakto:webhook] evento recebido: ${payload.event ?? "desconhecido"}`, payload);
  ...
  if (payload.event === "purchase_approved") {
    ...
    if (email) {
      try {
        ...
        if (error) {
          console.error("[creditOS:cakto:webhook] falha ao liberar acesso no Supabase", error);
        }
      } catch (error) {
        console.error("[creditOS:cakto:webhook] erro inesperado ao acionar Supabase", error);
      }
    }
  }
