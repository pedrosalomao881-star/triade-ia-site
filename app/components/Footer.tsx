import Link from "next/link";
import { WA_DIAGNOSTICO } from "@/lib/whatsapp";

const solucoes = [
  { href: "/atendimento", label: "Atendimento Inteligente" },
  { href: "/vendas", label: "Vendas Inteligentes" },
  { href: "/fiscal", label: "Inteligência Fiscal" },
  { href: "/sites", label: "Sites de Conversão" },
  { href: "/agronegocio", label: "IA para Agronegócio" },
];

const empresa = [
  { href: "/diagnostico", label: "Diagnóstico AI-First" },
  { href: "/quiz", label: "Quiz de Maturidade" },
  { href: "/sobre", label: "Sobre a TRÍADE IA" },
];

export default function Footer() {
  return (
    <footer className="border-t border-white/[0.07] mt-0">
      <div className="max-w-6xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
        {/* Brand */}
        <div className="md:col-span-2">
          <p className="font-display font-bold text-xl text-ice mb-3">
            TRÍADE <span className="text-brand">IA</span>
          </p>
          <p className="text-ice/35 text-sm leading-relaxed max-w-xs mb-5">
            Consultoria AI-First. Diagnóstico, estratégia e implementação
            com resultado mensurável — baseados em Itumbiara, Goiás.
          </p>
          <p className="text-ice/25 text-xs">
            Itumbiara, Goiás · Sul de Goiás
          </p>
        </div>

        {/* Soluções */}
        <div>
          <p className="text-xs font-semibold text-ice/25 uppercase tracking-widest mb-5">
            Soluções
          </p>
          <ul className="flex flex-col gap-2.5">
            {solucoes.map((s) => (
              <li key={s.href}>
                <Link
                  href={s.href}
                  className="text-sm text-ice/45 hover:text-ice transition-colors"
                >
                  {s.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Empresa + Contato */}
        <div>
          <p className="text-xs font-semibold text-ice/25 uppercase tracking-widest mb-5">
            Empresa
          </p>
          <ul className="flex flex-col gap-2.5 mb-8">
            {empresa.map((e) => (
              <li key={e.href}>
                <Link
                  href={e.href}
                  className="text-sm text-ice/45 hover:text-ice transition-colors"
                >
                  {e.label}
                </Link>
              </li>
            ))}
          </ul>

          <p className="text-xs font-semibold text-ice/25 uppercase tracking-widest mb-4">
            Contato
          </p>
          <a
            href={WA_DIAGNOSTICO}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-sm text-brand hover:text-brand-dark transition-colors font-medium"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            WhatsApp
          </a>
        </div>
      </div>

      <div className="border-t border-white/[0.06] px-6 py-4">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
          <p className="text-xs text-ice/20">
            © {new Date().getFullYear()} TRÍADE IA. Todos os direitos reservados.
          </p>
          <p className="text-xs text-ice/20">
            Consultoria AI-First · Itumbiara, Goiás
          </p>
        </div>
      </div>
    </footer>
  );
}
